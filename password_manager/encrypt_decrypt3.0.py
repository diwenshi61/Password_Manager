...
def encrypt():
    ...
    while password != y:
        ...
        y = hashlib.sha256(getpass.getpass().encode()).hexdigest()
        if password != y:
            print ("Try again")
            print ("")
    try:
                ...
                if char != "\n":
                    while True:
                        for i in range(LOOPS):
                            temp = hashlib.sha256(hashbase.encode()).hexdigest()
                            hashbase = temp
...
if __name__ == "__main__":
    while True:
        choice = input("Type 1 for encryption, 2 for decryption, 3 to remove passwords.txt, quit to quit\n")
        if choice == "1":
            encrypt()
        elif choice == "2":
            decrypt()
